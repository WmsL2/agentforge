...
└── commands/             # Click CLI commands (auto-discovered by `agentforge cmd …`)
...
The generated project ships a Click CLI exposed as `agentforge` (after `make install`):

```bash
agentforge server run --reload          # dev server
agentforge db upgrade                   # apply migrations
agentforge db migrate -m "message"      # create new migration
agentforge user create-admin            # interactive admin creation
agentforge celery worker                # start worker
agentforge celery beat                  # start scheduler
```

Run `make help` for a categorized list, or `agentforge --help` for full CLI docs.
